...
func TestDownloadURLPattern(t *testing.T) {
	t.Run("must build asset URLs that match the release naming (hyphens, not underscores)", func(t *testing.T) {
		got := fmt.Sprintf(downloadURLPattern, "v0.1.0-rc-02", "linux", "amd64")
		want := "https://github.com/eliasmeireles/stackctl/releases/download/v0.1.0-rc-02/stackctl-linux-amd64"
...
func TestFetchLatestStableVersion(t *testing.T) {
	t.Run("given a list with prereleases first then returns the first stable tag", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			_, _ = w.Write([]byte(`[
				{"tag_name": "v0.1.0-rc-05", "prerelease": true,  "draft": false},
				{"tag_name": "v0.1.0-rc-04", "prerelease": true,  "draft": false},
				{"tag_name": "v0.0.9",       "prerelease": false, "draft": false},
				{"tag_name": "v0.0.8",       "prerelease": false, "draft": false}
			]`))
		}))
		defer srv.Close()
		...
		require.Equal(t, "v0.0.9", version)
	})
...
func TestDownloadToTemp(t *testing.T) {
	...
	t.Run("given unwritable target dir then falls back to default temp dir", func(t *testing.T) {
		...
		path, err := downloadToTemp(srv.URL+"/binary", "/nonexistent-dir-for-stackctl-test")
		require.NoError(t, err)
		require.NotEmpty(t, path)
		require.NoError(t, os.Remove(path))
	})
...
func TestReplaceBinary(t *testing.T) {
	...
	t.Run("when dst dir does not exist then returns error", func(t *testing.T) {
		...
		err := replaceBinary(src, "/nonexistent-dir-for-stackctl-test/dst")
		require.Error(t, err)
	})
}

func TestPermissionHint(t *testing.T) {
	t.Run("when err is permission denied then suggests sudo stackctl self-update", func(t *testing.T) {
		err := permissionHint("install binary", "/usr/local/bin/stackctl", os.ErrPermission)
		require.Error(t, err)
		require.Contains(t, err.Error(), "sudo stackctl self-update")
	})

	t.Run("when err is not permission then wraps with action context", func(t *testing.T) {
		err := permissionHint("install binary", "/x", fmt.Errorf("disk full"))
		require.Error(t, err)
		require.Contains(t, err.Error(), "install binary")
		require.Contains(t, err.Error(), "disk full")
	})
}

func TestNewCommand(t *testing.T) {
	t.Run("must register the expected flags", func(t *testing.T) {
		cmd := NewCommand()
		require.Equal(t, "self-update", cmd.Use)
		require.NotNil(t, cmd.Flags().Lookup("version"))
		require.NotNil(t, cmd.Flags().Lookup("install-path"))
		require.Equal(t, defaultInstallPath, cmd.Flags().Lookup("install-path").DefValue)
	})
}
